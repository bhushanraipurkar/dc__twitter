...
const PTabNavigator = ({state,setState}: navigatorTypes) => {
  return (
    <div className={styles.pt__container}>
      <div className={`${styles.pt__tab} ${(state===0)?styles.selected__tab:""}`}>
        <p onClick={()=>setState(0)}>Tweets</p>
      </div>
      <div title="Tweets_&_replies" 
       className={`${styles.pt__tab} ${(state===1)?styles.selected__tab:""}`}
      >
        <p onClick={()=>setState(1)}>Tweets & replies</p>
      </div>
      <div title="Media" 
       className={`${styles.pt__tab} ${(state===2)?styles.selected__tab:""}`}
      >
        <p onClick={()=>setState(2)}>Media</p>
      </div>
      <div title="Likes" 
       className={`${styles.pt__tab} ${(state===3)?styles.selected__tab:""}`}
      >
